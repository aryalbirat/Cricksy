...
const initiateKhaltiPayment = async ({ bookingId, totalPrice, user }) => {
  try {
    ...
    const paymentData = {
      ...
      purchase_order_name: "Cricksal Booking",
      customer_info: {
...
//   try {
//     const user = req.user;
//     const cricksals = await Cricksal.find({ createdBy: user._id }).select('_id');
//     console.log('Cricksal arenas found:', cricksals);

//     if (!cricksals || cricksals.length === 0) {
//       return res.status(404).json({ error: 'You do not own any cricksal arenas' });
//     }

//     const arenaIds = cricksals.map((cricksal) => cricksal._id);
//     const payments = await Payment.find({ cricksalArena: { $in: arenaIds } })
//       .populate('userId', 'FirstName LastName')
//       .populate('bookingId', 'bookingDate startTime endTime status')
//       .select('userId bookingId amount paymentStatus transactionId paymentIdx paymentDate createdAt bookingDate startTime endTime cricksalArena');
...
const getPaymentsForArena = async (req, res) => {
  try {
    ...
    // Find cricksal arenas owned by user
    const cricksals = await Cricksal.find({ createdBy: user._id }).select('_id');
    if (!cricksals || cricksals.length === 0) {
      return res.status(404).json({ error: 'You do not own any cricksal arenas' });
    }
...
//       .select('userId bookingId amount paymentStatus transactionId paymentDate paymentIdx cricksalArena');
