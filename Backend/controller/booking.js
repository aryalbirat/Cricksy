...
// };
  ...
  //   try {
  //     // Fetch bookings for the authenticated user and populate cricksal details
  //     const bookings = await Booking.find({ user: req.user._id })
  //       .populate("cricksalArena", "name location images") // Populate required fields
...
//     // Fetch all bookings, filtering by the cricksal arenas owned by the logged-in user
//     const bookings = await Booking.find()
//       .populate({
//         path: 'cricksalArena',
//         match: { createdBy: req.user._id }, // Ensure the cricksal arena is created by the logged-in user
//         select: 'name location'
//       })
//       .populate('user', 'FirstName LastName Email') // Populate user data
//       .sort({ bookingDate: 1, startTime: 1 });

//     // Filter out null entries where cricksalArena didn't match the owner
//     const ownerBookings = bookings.filter(booking => booking.cricksalArena);
...
//     // Step 1: Find cricksal arenas owned by the user matching the search
//     const cricksals = await Cricksal.find({
//       createdBy: req.user._id,
//       name: { $regex: search, $options: "i" },
//     }).select("_id");

//     const cricksalIds = cricksals.map((f) => f._id);

//     if (!cricksalIds.length) {
//       return res.status(404).json({ message: "No cricksal arenas found matching the search." });
//     }

//     // Step 2: Find bookings that match those cricksal IDs
//     const bookingsQuery = Booking.find({
//       cricksalArena: { $in: cricksalIds },
//     })
...
//     if (!bookings.length) {
//       return res.status(404).json({ message: "No bookings found for the matching cricksal arenas." });
//     }
...
//     // Fetch all bookings with populated cricksal and user details
//     const bookings = await Booking.find()
//       .populate({
//         path: 'cricksalArena',
...
//     // Step 1: Find cricksal arenas matching the search term
//     const cricksals = await Cricksal.find({
//       name: { $regex: search, $options: "i" },
//     }).select("_id");
...
//     // If no cricksal arenas match the search, return early
//     if (!cricksalIds.length) {
//       return res.status(404).json({ message: "No cricksal arenas found matching the search." });
//     }

//     // Step 2: Find bookings for matching cricksal arenas
//     const bookingsQuery = Booking.find({
//       cricksalArena: { $in: cricksalIds },
//     })
//       .populate({
